...
import jrpricing.surcharge.domain.surcharge.BasicSurcharge
...
class SeasonVariationAmountTest {
    ...
    fun `繁忙期に乗車する場合は、指定席特急料金に200円加算された金額が返される`() {
        val basicSurcharge = BasicSurcharge(Amount.of(1000))
        val departureMonthDay = TestDepartureMonthDayFactory.createPeakDepartureMonthDay()

        val seasonVariationAmount = SeasonVariationAmount(basicSurcharge)
